...
public class MetaUtils {
    public static class TableBuilder {
        ...
        public TableBuilder addColumn(String name, DataType type, boolean pk) {
            ...
            TiColumnInfo col = new TiColumnInfo(
                    newId(),
                    name,
                    columns.size(),
                    type,
        ...
        public TableBuilder appendIndex(String indexName, List<String> colNames, boolean isPk) {
            List<TiIndexColumn> indexCols = colNames.stream()
                    .map(name -> columns.stream()
                                        .filter(c -> c.matchName(name))
                                        .findFirst())
                    .flatMap(col -> col.isPresent() ? Stream.of(col.get()) : Stream.empty())
                    .map(TiColumnInfo::toIndexColumn).collect(Collectors.toList());
